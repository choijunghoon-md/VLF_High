...
void Right(int t,float sp){
  ...
  */
  ...
  analogWrite(L_InPin_1,L_sp);  //왼쪽모터1 핀에 아날로그로 L_sp스피드의 출력을 넣어라
  analogWrite(L_InPin_2,0);   //왼쪽모터2 핀에 아날로그 신호를 0 넣어라
  analogWrite(R_InPin_1,R_sp);  //오른쪽모터1 핀에 아날로그로 R_sp스피드의 출력을 넣어라
  analogWrite(R_InPin_2,0);   //왼쪽모터2 핀에 아날로그 신호를 0 넣어라
...
void loop() {
 int R_moter_speed,L_moter_speed;
  /*
  F_distance=(F_sensor.ping_cm())*10;  //정면 초음파 센서값을 cm단위 정수로 F_distance에 저장_mm단위
  L_distance=(L_sensor.ping_cm())*10;  //왼쪽 초음파 센서값을 cm단위 정수로 L_distance에 저장_mm단위
  R_distance=(R_sensor.ping_cm())*10;  //오른쪽 초음파 센서값을 cm단위 정수로 R_distance에 저장_mm단위
  */
  ...
  */
  ...
  if(F_distance<=170 && F_distance!=0){
    ...
    /*if(R_distance > L_distance){runing(1,170,70);}
    else if(R_distance < L_distance){runing(1,70,170);}
    else {runing(1,80,95);}*/
    runing(1,80,95);
    }  
