  ...
  void wall_fillowing2(){ 
    ...
    if(L_Sonar_distance >= 1400){
      wall_fillowing1();
      }
    
    else if(dw+er*2>=R_Sonar_distance+L_Sonar_distance && R_Sonar_distance==L_Sonar_distance){
      wall_fillowing1();
      }
      ...
      if(ck==0){
        while(1){
          ...
          read_sonar_sensor();
          Left(1,65);
          if(dw+er*6>=R_Sonar_distance+L_Sonar_distance || index != 0){
            break;
          }
        }
      }
  ...
  void loop() {
    ...
    //  Serial.println(F_Sonar_distance);
      if(F_Sonar_distance<=370+er){
        while(1){
        index=read_line_sensor();
        read_sonar_sensor();
        Right(1,65);
        if(370>=L_Sonar_distance || index != 0){
          break;
        }
       }
      }  
