...
int error_R_L = R_Sonar_distance-L_Sonar_distance;
...
void loop() {
  ...
  //runing(1,nm_sp_L+(error_R_L)*p_gain,nm_sp_R-(error_R_L)*p_gain);


  if(dw-er<=(R_Sonar_distance+L_Sonar_distance) && dw+er<=(R_Sonar_distance+L_Sonar_distance)){
    if(R_Sonar_Error > 0){
      nm_sp_L++;
      nm_sp_R--;
      }
    ...
    runing(1,nm_sp_L+(R_Sonar_Error*p_gain),nm_sp_R-(R_Sonar_Error*p_gain));
    }
  else if(R_Sonar_distance == L_Sonar_distance){
    if(ck==0){
      while(1){
        read_sonar_sensor();
        Left(1,65);
